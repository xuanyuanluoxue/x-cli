...
> **对应测试**: `tests/test_config.py` + `tests/test_logging.py`（单元）+ `tests/test_e2e_config.py`（E2E）
...
- 不覆盖已存在的文件；需要覆盖时由用户先备份并移走旧文件

> **设计选择**：`x --config-init` 用来生成默认配置。`--config <path>` 则加载用户指定的现有配置。
...
| 现有 | 新增 | 关系 |
|---|---|---|
| `x todo` 子命令 | `x --config-init` | 全局选项（仅生成配置文件）|
