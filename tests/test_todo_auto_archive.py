...
def test_bdd_scenario_1_yaml_enable_list_archives_overdue_and_prints_summary(
    ...
    archived_dir = store.archive_dir / "20260626-kemu1"
...
def test_bdd_scenario_3_default_disabled_does_not_archive(
    ...
    assert not (store.archive_dir / "20260626-kemu1").exists()
...
def test_bdd_scenario_4_yaml_enable_stats_archives_and_counts_correctly(
    ...
    assert (store.archive_dir / "20260626-kemu1").is_dir()
    assert (store.archive_dir / "20260626-kemu2").is_dir()
    assert "reason: expired" in (
        (store.archive_dir / "20260626-kemu1" / "TODO.md").read_text(
            encoding="utf-8"
        )
    )
...
def test_bdd_scenario_5_yaml_enable_search_excludes_archived_overdue(
    ...
    assert any(
        name.startswith("20260626-") and "科目一" in name
...
def test_bdd_scenario_6_env_var_enables_without_yaml(
    ...
    assert (store.archive_dir / "20260626-kemu1").is_dir()
    frontmatter = (
        (store.archive_dir / "20260626-kemu1" / "TODO.md")
