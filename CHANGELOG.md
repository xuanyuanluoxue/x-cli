...
### Changed
- 中英文 README 新增「与 x-cmd 的关系」一节：明确两个 `x` 项目的定位
  （使用方式可能冲突、功能互补、环境变量与数据目录不冲突），并说明上游
  合作（讨论 #455、PR #456）与未来合并/并存的可能。
- 重写中英文 README，明确区分已发布的 v0.8.0、审核中的 WinGet 渠道和
  审核通过后才公布的一键安装命令。

### Fixed
- Windows 凭证库原子替换遇到防病毒、索引或文件系统的瞬时占用时执行有界
  退避重试，避免并发写入偶发 WinError 5，同时保留持续错误和旧 DB 内容。
