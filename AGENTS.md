...
Web frontend source lives only in `web/`; `core/web/static/` is committed build output and must not be edited by hand. After changing `web/`, run `npm ci` and `npm run build` from that directory, include the regenerated static assets in the same change, and verify `git status --short -- core/web/static` contains only the intentional rebuild. CI repeats the locked build and fails when committed artifacts do not match source.
...
`main` 是唯一长期分支。提交信息统一使用中文，格式为 `类型(模块): 描述`，例如 `feat(todo): 新增导出过滤功能` 或 `fix(web): 修复令牌验证失败`。每次提交保持聚焦。从最新 `origin/main` 拉短期 `feature/<name>` 或 `fix/<name>` 分支开发；禁止直接推送到 `main`。PR 目标为 `main`，默认 squash merge，需说明行为变更、引用相关规格或 issue，并附带测试和覆盖率结果。合并后删除短期分支，不再创建长期 `dev` 或 release 分支。未经用户明确授权，禁止提交、推送、amend 或绕过 hooks。
